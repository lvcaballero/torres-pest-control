...
begin
  ...
  if exists (select item_id from jsonb_to_recordset(p_items) as entries(item_id uuid, amount numeric) group by item_id having count(*) > 1) then raise exception 'Each inventory item can only be added once per stock-out.'; end if;
  ...
  for item_row in select item_id, amount from jsonb_to_recordset(p_items) as entries(item_id uuid, amount numeric) order by item_id loop
