...
export async function updateClient(clientId, form) {
  ...
  const { data, error } = await supabase
    .from("clients")
    .update({ ...buildClientPayload(form), version: Number(expectedVersion || 1) + 1 })
    .eq("id", clientId)
    .eq("version", Number(expectedVersion || 1))
    .select(CLIENT_COLUMNS)
    .single();

  if (!error && !data) return { error: "This client changed while you were editing. Reload before saving." };
  if (error?.code === "PGRST116") return { error: "This client changed while you were editing. Reload before saving." };
  if (error) return { error: describeError(error) };
  return { client: mapClientRow(data) };
}

export async function findPotentialDuplicateClients(form) {
  const email = form.email?.trim().toLowerCase();
  const phone = form.phone?.trim();
  let query = supabase.from("clients").select(CLIENT_COLUMNS).eq("status", "ACTIVE");
  if (email || phone) {
    const filters = [];
    ...
    query = query.ilike("name", form.name?.trim() || "");
  }
  const { data, error } = await query.limit(5);
  if (error) return { error: describeError(error), clients: [] };
  return { error: null, clients: (data || []).map((row) => mapClientRow(row)) };
}

export async function archiveClient(clientId, version) {
  const { data, error } = await supabase
    .from("clients")
    .update({ status: "ARCHIVED", archived_at: new Date().toISOString(), version: Number(version || 1) + 1 })
    .eq("id", clientId)
    .eq("version", Number(version || 1))
    .select(CLIENT_COLUMNS)
    .single();
  if (error?.code === "PGRST116") return { error: "This client changed while you were editing. Reload before archiving." };
  if (error) return { error: describeError(error) };
  return { client: mapClientRow(data) };
}
