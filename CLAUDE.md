...
- **`src/hooks/*`** — thin wrappers (`useAuth`, `useClients`, `useInventory`, `useServices`, `useUsers`, `useTreatmentMethods`, `useLogs`) that re-export a context and add derived helpers. Components should use these, not `useXContext` directly.
- **`src/utils/constants.js`** — single source of truth for roles, statuses, allowed upload types/sizes, document and attachment categories, and `COMPANY` letterhead details. Several of these mirror SQL `check` constraints or trigger logic; changing one side requires a migration on the other (`APPOINTMENT_STATUS_TRANSITIONS` ↔ migration 027, `DOCUMENT_CATEGORIES` ↔ 031, `ATTACHMENT_CATEGORIES` ↔ 029, `STOCK_OUT_REASONS` ↔ 040, `SERVICE_FREQUENCIES` ↔ 041, `LIMITS` ↔ 047).
...
- **`src/utils/expiry.js`** — which deliveries of an item are still on the shelf (`lotsOnHand`, first-in-first-out against `inventory.quantity`) and the nearest expiry among them, for the item detail and the expiring-soon badge. Pure, unit-tested.
...
Migration 048 moves the expiration date from the item to the delivery: `stock_in_batch()` takes `expiration_date` and `batch_number` per line (chemicals only, not before the delivery date) and writes them on the IN movement. `inventory.expiration_date` is kept but no longer written; 048 copied each value onto that item's latest delivery, and the app only falls back to it for an item with no dated delivery. Re-running 040 restores a `stock_in_batch` that silently drops both fields; re-run 048 after it.
